...
src/
├── components/
│   ├── BottomNav.jsx        # Fixed 6-tab navigation bar
...
│   ├── ClientsPage.jsx      # Client list + full ManageClientModal
│   ├── MeasurementsPage.jsx # Body measurement logging with deltas
│   ├── PRLogPage.jsx        # PR log + custom exercise management
...
Six tabs, no router. `App.jsx` holds `tab` state and renders the active page conditionally:

```
Clients · PRs · Attributes · Measurements · Rankings · Attendance
```

Settings opens as a modal overlay triggered from the nav bar gear icon, not a tab.

Pages that operate on a selected client receive `{ clientId, setClientId }` as props from `App.jsx`. `RankingsPage` and `AttendancePage` are standalone — they load their own data.
...
/clients/{clientId}
  ...
  membershipType:      3 | 4 | 5           // classes per week
...
Expected attendance counts use **28-day rolling cycles anchored to each client's `startDate`**, not calendar months. This solves a real problem: a client who joins mid-month would show artificially low attendance against a full calendar-month expectation.
